...
public class ImageMessage {
    ...
    public static ImageMessage create(String url, List<String> message) {
        AtomicReference<ImageMessage> m = new AtomicReference<>();
        Task.submitAsynchronously(-1, -1, c -> {
            try {
                ImageMessage message1 = new ImageMessage(ImageIO.read(new URL(url)), 8, ImageChar.BLOCK.getChar());
                message1.appendText(PMath.toStringArray(message));
                m.set(message1);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static ImageMessage create(URL url, ImageChar imageChar) {
        AtomicReference<ImageMessage> m = new AtomicReference<>();
        try {
            m.set(new ImageMessage(ImageIO.read(url), 8, imageChar.getChar()));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return m.get();
