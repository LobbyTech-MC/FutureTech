...
public class FT_SIRIROBOT extends Item<SlimefunItemStack> {
    ...
    private void replay(String message, AsyncPlayerChatEvent event){
        new BukkitRunnable(){
            @Override
            public void run() {
                try {
                    ...
                    for (Entity entity : event.getPlayer().getLocation().getNearbyEntities(10,10,10)){
                        if (entity instanceof Player){
                            PMessage.to((Player) entity,"&cSiri &f对 &c" + event.getPlayer().getName() + " &f说:&9" + replyMessage);
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
